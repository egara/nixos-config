...
- [x] **Selector de Wallpapers (Galería Visual):** Pill dedicado en la barra (`󰋩`) a la izquierda del de Power Management con selector visual en cuadrícula de wallpapers desde `~/.config/sicos/wallpapers` (incluyendo subcarpetas y symlinks), visor interactivo de ruta con enlace a Nautilus, buscador en vivo, filtrado por categorías de carpetas, cambio instantáneo mediante `awww` y soporte de wallpaper aleatorio con click central.
- [x] **OSD de Feedback Multi-Monitor (Target OSD):** Pill compartido estilo volumen (`targetosd.nix`) instanciado por pantalla que muestra `󰍹 <label> | <valor>` únicamente en el monitor objetivo de una acción: al seleccionar output en la galería de wallpapers (`Output | DP-1`, uno por pantalla con "All Outputs") y al cambiar/aplicar escala en el Control Center (`Scale | DP-1 · 1.25x`). API global `mainScope.showTargetOsd(outputs, label, value)` con auto-ocultado a 2s; cualquier nuevo ajuste multi-monitor debe reutilizarla.
